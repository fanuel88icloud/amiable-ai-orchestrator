...
function ChannelEditorPage() {
  ...
  const verifyImapMutation = useMutation({
    mutationFn: () => verifyImapConnection(organizationId!, channelId),
    onSuccess: async () => {
      toast.success("Connessione IMAP e SMTP verificata");
      await emailConnectionQuery.refetch();
    },
    onError: (error: Error) => toast.error(error.message),
  });
              ...
              <CardContent className="space-y-3">
                {emailConnectionQuery.isLoading ? (
                  ...
                  <>
                    ...
                    {emailConnectionQuery.data.provider === "imap" && (
                      <Button
                        type="button"
                        onClick={() => verifyImapMutation.mutate()}
                        disabled={!canWrite || verifyImapMutation.isPending}
                      >
                        <RefreshCw
